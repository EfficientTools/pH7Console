...
use super::local_llm::{Capability, LocalModelInfo, ModelType};
...
impl LightweightLLM {
    ...
    pub async fn load_model(&mut self) -> Result<()> {
        ...
        println!(
            "🔄 Loading enhanced ML system with advanced natural language understanding: {}",
        ...
        self.is_loaded = true;
        println!(
            "✅ Enhanced ML system loaded with {} comprehensive command patterns",
            self.patterns.len()
        );
    ...
    pub async fn generate(&self, request: InferenceRequest) -> Result<LLMResponse> {
        ...
        let response_text = match request.capability {
            Capability::NaturalLanguageToCommand => {
                self.process_advanced_command_generation(
                    &request.prompt,
            ...
            Capability::ErrorAnalysis => self.process_error_analysis(&request.prompt).await?,
            Capability::CodeGeneration => self.process_code_generation(&request.prompt).await?,
            _ => self.process_general_query(&request.prompt).await?,
        };
        ...
        vec![
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "home path".to_string(),
                    "go to home".to_string(),
                    "navigate home".to_string(),
                    "change to home".to_string(),
                    "return home".to_string(),
                    "home base".to_string(),
                    "home location".to_string(),
                ],
                command_template: "cd ~".to_string(),
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "back directory".to_string(),
                    "one level up".to_string(),
                    "go back".to_string(),
                    "parent folder".to_string(),
                    "up directory".to_string(),
                    "previous directory".to_string(),
                    "go to parent".to_string(),
                    "move up".to_string(),
                    "step back".to_string(),
                    "level up".to_string(),
                    "back one".to_string(),
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "display files".to_string(),
                    "see files".to_string(),
                    "list directory".to_string(),
                    "show folder".to_string(),
                    "what files".to_string(),
                    "directory listing".to_string(),
                    "ls".to_string(),
                    "list all".to_string(),
                    "show all files".to_string(),
                    "view files".to_string(),
                    "what's in here".to_string(),
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "version control status".to_string(),
                    "check repository".to_string(),
                    "git check".to_string(),
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "commit files".to_string(),
                    "save to git".to_string(),
                    "record changes".to_string(),
                    "commit all".to_string(),
                    "save everything".to_string(),
                    "commit modifications".to_string(),
                ],
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "git push".to_string(),
                    "upload changes".to_string(),
                    "send to remote".to_string(),
                    "publish changes".to_string(),
                    "push to origin".to_string(),
                    "upload to github".to_string(),
                    "sync remote".to_string(),
                    "deploy changes".to_string(),
                ],
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "running programs".to_string(),
                    "active processes".to_string(),
                    "what's running".to_string(),
                    "process list".to_string(),
                    "running tasks".to_string(),
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "disk info".to_string(),
                    "space remaining".to_string(),
                    "drive space".to_string(),
                    "filesystem usage".to_string(),
                ],
            ...
            CommandPattern {
                triggers: vec![
                    ...
                    "install module".to_string(),
                    "get package".to_string(),
                    "fetch dependency".to_string(),
                    "download package".to_string(),
                ],
    ...
    async fn process_advanced_command_generation(
        &self,
        prompt: &str,
        context: Option<&str>,
    ) -> Result<String> {
        ...
        for pattern in &self.patterns {
            let example_strength = pattern
                .example_inputs
                .iter()
                .map(|example| self.calculate_match_strength(&prompt_lower, example))
            ...
            for trigger in &pattern.triggers {
                ...
                if match_strength > 0.5 {
                    ...
                    if example_strength > 0.7 {
                        confidence += 0.04 * example_strength;
                    }

                    // Boost for multiple trigger matches in same pattern
                    let trigger_matches = pattern
                        .triggers
                        .iter()
                    ...
                    let command = self.extract_smart_parameters(
                        &prompt_lower,
                        trigger,
        ...
        self.enhanced_fallback_processing(&prompt_lower, context)
            .await
    }
    ...
    fn has_similar_word_order(&self, prompt: &str, trigger: &str) -> bool {
        ...
        for trigger_word in &trigger_words {
            if let Some(pos) = prompt_words[last_found_index..]
                .iter()
                .position(|&w| w == *trigger_word)
            {
    ...
    fn extract_search_parameter(&self, prompt: &str) -> String {
        ...
        let extensions = [
            "txt", "js", "ts", "rs", "py", "json", "html", "css", "md", "log", "xml",
        ];
    ...
    fn extract_creation_parameter(&self, prompt: &str, template: &str) -> String {
        let cmd = if template.contains("touch") {
            "touch"
        } else {
            "mkdir"
        };
    ...
    async fn enhanced_fallback_processing(
        &self,
        prompt: &str,
        context: Option<&str>,
    ) -> Result<String> {
    ...
    async fn calculate_advanced_confidence(
...

        {
            ...
            if let Some(learned_score) = learning_stats.get(prompt) {
                confidence += (*learned_score - 0.5) * 0.2;
            }
        }
    ...
    pub async fn clear_adaptive_memory(&self) {
        self.cache.lock().await.clear();
        self.usage_stats.lock().await.clear();
        self.learning_stats.lock().await.clear();
    }
...
impl LLMFactory {
    pub async fn create_for_capability(capability: Capability) -> Result<LightweightLLM> {
        ...
        LightweightLLM::new(ModelType::LocalPatternEngine).await
    }
}
